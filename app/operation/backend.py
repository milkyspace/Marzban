...
from app.models.backend import BackendCreate, BackendResponseList
...
class BackendOperation(BaseOperator):
    async def add_backend(self, db: AsyncSession, new_backend: BackendCreate, admin: AdminDetails) -> BackendConfig:
        try:
        ...
        logger.info(f'Backend config "{db_backend.name}" created by admin "{admin.username}"')
        return db_backend

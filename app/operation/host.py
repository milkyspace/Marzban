...
class HostOperator(BaseOperator):
    async def get_hosts(self, db: Session, offset: int = 0, limit: int = 0) -> list[BaseHost]:
        return await get_hosts(db=db, offset=offset, limit=limit)

    async def add_host(self, db: Session, new_host: CreateHost, admin: Admin) -> BaseHost:
    ...
    async def modify_host(self, db: Session, host_id: int, modified_host: CreateHost, admin: Admin) -> BaseHost:
    ...
    async def remove_host(self, db: Session, host_id: int, admin: Admin):
        db_host = await self.get_validated_host(db, host_id)
